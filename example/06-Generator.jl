...
x = collect(i.name for i in QueryOperators.query(df) if i.age>30.)
println(x)

x = collect(@NT(Name=i.name) for i in QueryOperators.query(df) if i.age>30.)
println(x)

x = collect(QueryOperators.query(x), DataFrame)
println(x)
