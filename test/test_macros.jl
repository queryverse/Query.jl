...
@testset "Select Macro" begin
    ...
    @test_skip df |> Query.@select(rangeat(foo, bar), endswith) |> DataFrame == DataFrame(foo=[1,2,3], bar=[3.,2.,1.])

    @test df |> Query.@select(startswith(b), endswith(r)) |> DataFrame == DataFrame(bar=[3.,2.,1.],)
    @test_skip df |> Query.@select(occursin(o), startswith(ba)) == DataFrame()
