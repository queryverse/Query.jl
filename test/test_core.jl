...
@testitem "Main tests" begin
    ...
    q = collect(Query.QueryOperators.default_if_empty(DataValue{String}[]))
    @test length(q)==1
    @test isna(q[1])

    q = collect(Query.QueryOperators.default_if_empty(DataValue{String}["John", "Sally"]))
    @test length(q)==2
